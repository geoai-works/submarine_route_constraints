...
| `05_assign_constraint_scores.ipynb`       | Compute Exclusion Scores by Grid Cell | Applies the full scoring logic (hard + soft constraints) and exports the final grid         |
...
- `05_grid_with_exclusion_scores.gpkg`: Main output with per-cell scores and flags  
...
> Output files generated by the notebooks — including `05_grid_with_exclusion_scores.gpkg` — are **not included** in the repository due to GitHub file size limitations (some exceed 100 MB).  
...
- `05_assign_constraint_scores.ipynb`
